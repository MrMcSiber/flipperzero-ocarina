...
void draw_callback(Canvas *canvas, void *ctx) {
  Ocarina *ocarina = ctx;
  furi_check(furi_mutex_acquire(ocarina->model_mutex, FuriWaitForever) ==
             FuriStatusOk);
...
void ocarina_free(Ocarina *instance) {
  ...
  gui_remove_view_port(
      instance->gui, instance->view_port); // Removes our ViewPort from the Gui
  furi_record_close("gui");                // Closes the gui record
  view_port_free(
      instance->view_port); // Frees memory allocated by view_port_alloc
...
int32_t ocarina_app(void *p) {
  ...
  for (bool processing = true; processing;) {
    ...
    FuriStatus status =
        furi_message_queue_get(ocarina->event_queue, &event, 100);
    furi_check(furi_mutex_acquire(ocarina->model_mutex, FuriWaitForever) ==
               FuriStatusOk);
